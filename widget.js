        ...
        onMouseUp: function (event) {
            ...
            if (intersects.length > 0) {
                ...
                if (obj != that.lastIntersect) {
                    ...
                    // TODO: Maybe we mark this in an extra hash
                    //        this.displayDrops['LED1'] = true // false
               ...
               clippers[keyname].smds.forEach(function(smd){
                  ...
                  var group = new THREE.Object3D();//create an empty container
                   ...
                   if(s.dx >= diameter*2 || s.dy >= diameter*2){
                        ...
                        var startx = 0-(s.dx/2) + radius + (space_x/2);
                        var starty = 0-(s.dy/2) + radius + (space_y/2);
                        ...
                        for(var iy=1; iy <= steps_y; iy++){
                           ...
                           for(var ix=1; ix <= steps_x;ix++){
                              var drop = PARENT.drawSphere(posx, posy, (that.cannulaDiameter/2), that.colorsDrop[0]);
                              group.add( drop );//add a mesh with geometry to it
                              posx += diameter + space_x;
                           }
                        ...
                        if (smd.threeObj.userData.elem.rot) {
                           ...
                           if(s.rot){
                              var srot = parseInt(s.rot.replace(/\D+/,''));
                              rot += srot;
                           }
                        ...
                        var startxGroup = vector.x;//-(s.dx/2) + radius + (space_x/2);
                        group.position.setX(startxGroup);
                        var startyGroup = vector.y;//-(s.dy/2) + radius + (space_y/2);
                        group.position.setY(startyGroup);
                   }  else {
...
console.log(that.namedDropGroups);
        ...
        exportGcodeDispenser:function(PARENT){
            ...
            g += "(------ DISPENSER DROP's -------)\n";
            g += "M5 (spindle stop)\n";
            if(this.options.DispenserXoffset !== undefined)
                g += "G92 X" + this.options.DispenserXoffset + " Y" + this.options.DispenserYoffset + "\n"; 
